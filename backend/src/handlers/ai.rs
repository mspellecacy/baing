...
async fn get_discovery_movies_rand_n(
    ...
    let query_type = match !dq.query.is_empty() {
        false => ai_movie::get_random(&data.chatgpt, count, user_special_collections.to_owned()).await,
        true => ai_movie::get_guided(&data.chatgpt, count, user_special_collections.to_owned(), &dq.query).await,
    };
...
async fn get_discovery_tv_shows_rand_n(
    ...
    let query_type = match !dq.query.is_empty() {
        ...
        true => ai_tv::get_guided(&data.chatgpt, count, user_special_collections.to_owned(), &dq.query).await,
    };
